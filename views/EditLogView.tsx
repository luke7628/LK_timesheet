...
const EditLogView: React.FC<EditLogViewProps> = ({ contract, log, onBack, onSubmit }) => {
  const [description, setDescription] = useState(log.description);
  const [duration, setDuration] = useState(log.duration.split(' ')[0]);
  const [showTimePicker, setShowTimePicker] = useState(false);
  const [showDatePicker, setShowDatePicker] = useState(false);

  // Initial picker states
  const initialHours = Math.floor(parseFloat(duration));
  const initialMinutes = Math.round((parseFloat(duration) - initialHours) * 60);
  const [selectedHour, setSelectedHour] = useState(initialHours);
  const [selectedMinute, setSelectedMinute] = useState(initialMinutes);

  const [displayDate, setDisplayDate] = useState(log.date);
  ...
  const handleSubmit = () => {
    ...
    onSubmit({ description, duration: (selectedHour + (selectedMinute / 60)).toFixed(2), date: displayDate });
  };
  ...
  return (
    <div className="flex-1 flex flex-col bg-background-light dark:bg-background-dark h-full relative overflow-hidden">
      <header className="sticky top-0 z-10 flex items-center bg-white dark:bg-background-dark p-4 border-b border-gray-100 dark:border-white/5 justify-between safe-top">
        ...
        <h2 className="text-text-dark dark:text-white text-[14px] font-black uppercase tracking-widest">Edit Log</h2>
      ...
      <main className="flex-1 overflow-y-auto pb-32 no-scrollbar px-4 pt-4">
        ...
        <div className="space-y-4">
          <div className="bg-white dark:bg-neutral-900 rounded-3xl shadow-sm border border-gray-100 dark:border-white/5 overflow-hidden">
            <button onClick={() => setShowDatePicker(true)} className="w-full flex items-center gap-4 px-5 py-5 justify-between border-b border-gray-50 active:bg-gray-50 transition-colors">
            ...
            <button onClick={() => setShowTimePicker(true)} className="w-full flex items-center gap-4 px-5 py-5 justify-between active:bg-gray-50 transition-colors">
              <div className="flex items-center gap-3">
              ...
              <span className="text-sage-green font-black text-sm bg-sage-green/5 px-3 py-1.5 rounded-xl">{(selectedHour + (selectedMinute / 60)).toFixed(2)} <span className="text-[9px] opacity-40">H</span></span>
            </button>
          </div>
      ...
      {showTimePicker && (
        <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/70 backdrop-blur-sm" 
          ...
          <div className="w-full max-w-lg bg-white rounded-t-[3rem] px-6 pt-6 pb-12 shadow-2xl animate-fadeIn" onClick={(e) => e.stopPropagation()}>
            ...
            <button onClick={() => { setShowTimePicker(false); }} className="w-full h-16 bg-sage-green text-white rounded-[1.5rem] font-black uppercase tracking-widest shadow-xl active:scale-95 transition-all">Confirm Selection</button>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[110] flex items-end justify-center bg-black/70 backdrop-blur-sm" 
          ...
          <div className="w-full max-w-lg bg-white rounded-t-[3rem] px-6 pt-6 pb-14 shadow-2xl animate-fadeIn" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex items-center justify-between py-4 mb-2">
              <button onClick={() => setCalMonth(m => m === 0 ? 11 : m - 1)} className="size-10 rounded-2xl bg-sage-green/5 text-sage-green flex items-center justify-center active:scale-90 transition-all"><span className="material-symbols-outlined">chevron_left</span></button>
              ...
              <button onClick={() => setCalMonth(m => m === 11 ? 0 : m + 1)} className="size-10 rounded-2xl bg-sage-green/5 text-sage-green flex items-center justify-center active:scale-90 transition-all"><span className="material-symbols-outlined">chevron_right</span></button>
            </div>
            ...
            <div className="grid grid-cols-7 gap-1.5 px-1">
              ...
              {Array.from({ length: daysInMonth(calMonth, calYear) }).map((_, i) => {
                ...
                const isSelected = displayDate === dObj.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) || displayDate === `${String(day).padStart(2, '0')}/${String(calMonth+1).padStart(2, '0')}/${String(calYear).slice(-2)}`;
                ...
                return (
                  <button key={day} onClick={() => { 
                    ...
                    setDisplayDate(`${dd}/${mm}/${yy}`); 
                    setShowDatePicker(false); 
                  }} className={`h-11 rounded-2xl flex items-center justify-center text-[13px] font-bold transition-all ${isSelected ? 'bg-sage-green text-white shadow-lg' : 'text-gray-700 active:bg-sage-green/5'}`}>
                    {day}
                  </button>
                );
              })}
